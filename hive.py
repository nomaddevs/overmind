...
model.add(Conv2D(32, (3, 3), padding='same', input_shape=(176, 200, 3), ctivation='relu'))
...
def check_data():
	...
	print("Total data length is now:", total_data)
	return lengths
...
for i in range(hm_epochs):
	...
	while not maximum:
		print("Currently training {}:P{".format(current, current+increment))
		no_attacks = []
        ...
        model.save("BasicCNN-{}-epochs-{}-ALPHA-BUILD".format(hm_epochs, learning_rate))
        current += increment
        if current > maximum:
            not_maximum = False